...
async function main() {
  ...
  cols.forEach((x, i) => {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(batLabels[i], x, 702);

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 24px sans-serif';
    ctx.shadowColor = tc;
    ctx.shadowBlur = 8;
    ctx.fillText(batVals[i], x, 726);
    ctx.restore();
  });

  // Draw Bowling Stats Values
  // Row: label y = 818, val y = 842
  ...
  cols.forEach((x, i) => {
    ...
    ctx.font = 'bold 11px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(bowLabels[i], x, 818);

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 24px sans-serif';
    ctx.shadowColor = tc;
    ctx.shadowBlur = 8;
    ctx.fillText(bowVals[i], x, 842);
    ctx.restore();
  });
  ...
  botLefts.forEach((left, i) => {
    ...
    ctx.shadowColor = tc;
    ctx.shadowBlur = 8;
    ctx.fillText(botVals[i], textX, 926);
    ctx.restore();
  });

  fs.writeFileSync(path.join(__dirname, 'test_stats_alignment.png'), canvas.toBuffer('image/png'));
  console.log('Saved test_stats_alignment.png');
}
