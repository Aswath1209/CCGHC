...
module.exports = function installTourMode(bot, sleep, sendEventUpdate, COMMENTARY, CCL_GIFS, GIF_EVENTS) {
  ...
  function resolveActiveTriAndTour(ctx) {
      const tour = tourManager.getUserTour(ctx.from.id) || [...tourManager.getAllTours()].find(t => t.chatId === ctx.chat.id);
      let tri = null;
      if (tour && tour.triSeriesId) {
          tri = triManager.getTriSeries(tour.triSeriesId);
      }
      if (!tri) {
          tri = triManager.getTriSeries(ctx.chat.id) || triManager.getUserTriSeries(ctx.from.id);
      }
      return { tri, tour };
  }
  ...
  async function handleTriMatchEndMessage(ctx, triRes) {
      if (!triRes) return;
      const tri = triRes.tri;
      const match = triRes.match;
      ...
      if (match.isFinal) {
          ...
          const congratText = `🎉 <b>CONGRATULATIONS!</b> 🎉\n\n🏆 <b>${escapeHtml(champName)}</b> are the Champions of the Tri-Series Tournament! 🏆`;
          
          try {
              await ctx.api.sendAnimation(tri.chatId, randomGif, {
                  caption: congratText,
                  parse_mode: 'HTML'
              });
          } catch (gifErr) {
              console.log("Failed to send celebration GIF:", gifErr.message);
              await ctx.api.sendMessage(tri.chatId, congratText, { parse_mode: 'HTML' });
          }
          
          // 2. Send the results messages (schedule with results)
          const resultsText = `🏁 <b>TRI-SERIES TOURNAMENT RESULTS</b> 🏁\n\n` + renderTriScheduleText(tri);
          await ctx.api.sendMessage(tri.chatId, resultsText, { parse_mode: 'HTML' });
  ...
  bot.command(['status', 'tristatus'], async (ctx) => {
    const { tri } = resolveActiveTriAndTour(ctx);
  ...
  bot.command(['stats', 'tristats', 'leaders', 'trileaders'], async (ctx) => {
    ...
    text += `───────────────────\n`;
    runsList.forEach((p, idx) => {
      text += `${idx + 1}. <b>${escapeHtml(p.name)}</b>: ${p.runs} runs (${p.ballsFaced}b)\n`;
    });
    
    text += `\n🥎 <b>Most Wickets (Purple Cap)</b>\n`;
    text += `───────────────────\n`;
    wicketsList.forEach((p, idx) => {
      const ov = Math.floor(p.ballsBowled / 6);
      const bl = p.ballsBowled % 6;
      text += `${idx + 1}. <b>${escapeHtml(p.name)}</b>: ${p.wickets} wkts (${ov}.${bl} ov)\n`;
    });

    text += `\n✨ <b>Player of the Series (MVP Points)</b>\n`;
    text += `───────────────────\n`;
    potsList.forEach((p, idx) => {
      text += `${idx + 1}. <b>${escapeHtml(p.name)}</b>: ${Number(p.potsPoints.toFixed(2))} pts (runs: ${p.runs}, wkts: ${p.wickets})\n`;
    });
      ...
      if (tri) {
          ...
          if (res.success) {
              ...
              if (tri.activeTourId) {
                  const tour = tourManager.getTour(tri.activeTourId);
                  if (tour) {
                      const tourTeamKey = tour.teamA.triTeamKey === teamKey ? 'teamA' : (tour.teamB.triTeamKey === teamKey ? 'teamB' : null);
                      if (tourTeamKey) {
                          tourManager.appointCaptain(tour.id, ctx.from.id, targetUserId, tourTeamKey);
                      }
                  }
              }
  ...
  bot.command(['adda', 'addb', 'addc', 'addd', 'adde', 'addf', 'addg', 'addh', 'add'], async (ctx) => {
      ...
      const { tri, tour: activeTour } = resolveActiveTriAndTour(ctx);
      if (tri) {
          ...
          if (tri.state === 'LOBBY' || tri.state === 'SCHEDULED') {
              ...
              const tour = activeTour || tourManager.getTour(tri.activeTourId);
  ...
  bot.command('rebat', async (ctx) => {
      ...
      if (!teamArg || isNaN(index)) {
          if (tri) {
              const teamKeys = triManager.getTeamKeys(tri);
              const teamsList = teamKeys.map(k => `<b>${k.replace('team', '')}</b> (${escapeHtml(tri[k].name)})`).join(', ');
              return ctx.reply(`Usage: /rebat [teamLetter] [playerIndex]\n\nTeams: ${teamsList}\nExample: <code>/rebat B 1</code>`, { parse_mode: 'HTML' });
          } else {
              if (!tour) return ctx.reply("You are not in an active Tour match.");
              const nameA = tour.teamA.name;
              const nameB = tour.teamB.name;
              return ctx.reply(`Usage: /rebat [team] [playerIndex]\n\nTeams: <b>${escapeHtml(nameA)}</b> or <b>${escapeHtml(nameB)}</b>\nExample: <code>/rebat A 1</code>`, { parse_mode: 'HTML' });
          }
      }
      ...
      if (tri) {
          if (['A','B','C','D','E','F','G','H'].includes(teamArgUp)) {
              triTeamKey = 'team' + teamArgUp;
              if (!tri[triTeamKey]) {
                  return ctx.reply(`❌ Team ${teamArgUp} does not exist in this tournament.`);
              }
              if (tour) {
                  if (tour.teamA.triTeamKey === triTeamKey) {
                      teamKey = 'teamA';
                      teamChar = 'A';
                  } else if (tour.teamB.triTeamKey === triTeamKey) {
                      teamKey = 'teamB';
                      teamChar = 'B';
                  }
              }
              ...
              for (const k of teamKeys) {
                  if (tri[k].name.toLowerCase().includes(argLower)) {
                      triTeamKey = k;
                      if (tour) {
                          if (tour.teamA.triTeamKey === triTeamKey) {
                              teamKey = 'teamA';
      ...
      if (tri) {
          if (!triTeamKey) {
              return ctx.reply("❌ Team not found in the tournament.");
          }
          
          const team = tri[triTeamKey];
          if (ctx.from.id !== tri.hostId && ctx.from.id !== team.captainId) {
              return ctx.reply("Only the host or team captain can allow a rebat.");
          }
          
          const player = team.players[index - 1];
          if (!player) {
              return ctx.reply("❌ Player index not found in team roster.");
          }
          
          const rebatObj = {
              id: player.id + '_rebat_' + Date.now(),
              originalId: player.originalId || player.id,
              first_name: player.first_name.replace(/\s*\(rebat\)/gi, '') + " (rebat)",
              runs: 0,
              balls: 0,
              wickets: 0,
              runsConceded: 0,
          ...
          triManager.saveState();
          
          if (tour && teamKey) {
              tour[teamKey].players.push(rebatObj);
              
              const cleanName = rebatObj.first_name.replace(/\s*\(rebat\)/gi, '');
              ...
              const cleanName = rebatObj.first_name.replace(/\s*\(rebat\)/gi, '');
              await ctx.reply(`🔄 <b>${escapeHtml(cleanName)} (rebat)</b> has been registered in the <b>${escapeHtml(team.name)}</b> tournament roster (Not playing in current match)!`, { parse_mode: 'HTML' });
          }
      } else {
          if (!teamKey) {
              const nameA = tour.teamA.name;
  ...
  bot.command('tourhelp', async (ctx) => {
      await ctx.reply(
          ...
          "7. /tristats / /trileaders - View tournament stats and leaderboards\n",
          { parse_mode: 'HTML' }
      );
  });
          ...
          if (res.matchEnded) {
              ...
              if (tour.triSeriesId) {
                  try {
                      ...
                      await handleTriMatchEndMessage(ctx, triRes);
                  } catch (ptErr) {
                      console.error("Error handling Tri-Series match end:", ptErr);
                  }
              }
              ...
              if (data.startsWith('tri_fwconfirm_')) {
                  ...
                  if (triRes) {
                      try {
                          ...
                          console.error("Error handling Tri-Series match end after free win:", ptErr);
                      }
                  }
                  return;
              }
